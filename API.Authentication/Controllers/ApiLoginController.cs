...
namespace API.Authentication.Controllers
{
    ...
    public class ApiLoginController : Controller
    {
        ...
        [HttpPut("reset")]
        [Authorize]
        public async Task<IActionResult> ResetOwnPassword([FromBody] ResetOwnPasswordModel model)
        {
            if (ModelState.IsValid)
            {
                UserModel user = await _userManager.FindByEmailAsync(model.Email);
                ...
                ModelState.AddModelError("UserNotFound", $"User with email {model.Email} does not exist");
            }
